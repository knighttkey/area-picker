...
export interface SchedulePropType {
  defaultColor: string;
  activeColor: string;
  fontSize: string;
  cubeWidth: string;
...
const ScheduleHeader = styled.div`
  ...
  flex-direction: column;
`;
const ColumnsCubeWrap = styled.div.attrs(
...
`;
const ColumnsCube = styled.div.attrs(
...
const ScheduleBody = styled.div`
  ...
  justify-content: center;
`;
const RowsCubeWrap = styled.div`
  display: flex;
  justify-content: center;
  flex-direction: column;
  margin: 0 6px 0 0;
`;
const RowsCube = styled.div.attrs(
  (props: { cubeWidth: string; cubeHeight: string; fontSize: string, cubeMargin: string }) => props
)`
  min-width: ${(props) => props.cubeWidth};
  height: ${(props) => props.cubeHeight};
...
export const Schedule = (props: SchedulePropType) => {
  const {
    defaultColor,
    activeColor,
    fontSize,
  ...
  const pickAll = () => {
    ...
    allList.forEach((columnItem, columnKey: number) => {
      columnItem.forEach((rowItem, rowKey: number) => {
        allCubeId.push(`${columnKey + 1}-${rowKey + 1}`);
      });
    });
    if (selectMode) {
      setList(allCubeId);
  ...
  const pickCube = (eventTarget: ClickEventTarget) => {
    if (selectMode) {
      ...
      tempList = _.without(list, eventTarget.id);
      setList(tempList);
    }
  };
  let tempList = [...list];
  ...
  const renderColumnsName = () => {
    return (
      <ColumnsCubeWrap cubeHeight={cubeHeight}>
        <ColumnsCube
          style={{
            margin: "0 7px 0 3px",
            cursor: "pointer",
        ...
        >
          {dayItem.map((cubeItem, cubeKey) => {
            return (
              <Cube
                className={`cube ${
                  _.includes(list, `${key + 1}-${cubeKey + 1}`)
                    ? "selected"
                    : ""
                }`}
                id={`${key + 1}-${cubeKey + 1}`}
                ...
                cubeMargin={cubeMargin}
                style={{
                  backgroundColor: _.includes(list, `${key + 1}-${cubeKey + 1}`)
                    ? activeColor
                    : defaultColor
                }}
                onClick={(e) =>
  ...
  return (
    <ScheduleContainer>
      <ScheduleHeader>{renderColumnsName()}</ScheduleHeader>
      <ScheduleBody>
        {renderRowsName()}
        <Wrap
